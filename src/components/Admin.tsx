...
const inputStyle: React.CSSProperties = {
  width: "100%", padding: "8px", border: "var(--border-thin)", fontFamily: "var(--font-body)",
  background: "var(--c-card-bg)", color: "var(--c-black)", borderRadius: "4px", boxSizing: "border-box",
};
        ...
        <div key={s.key} style={{ padding: "10px", border: "var(--border-thin)", background: editing === s.key ? "var(--c-accent)" : "var(--c-grey-light)" }}>
          ...
          {editing === s.key ? (
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: "0.85rem", fontFamily: "monospace", wordBreak: "break-all" }}>{(settings)[s.key] || <span style={{ opacity: 0.3 }}>not set</span>}</span>
              <button onClick={() => { setEditing(s.key); setEditValue(settings[s.key] || ""); }} className="comic-btn" style={{ fontSize: "0.7rem", padding: "4px 10px" }}>EDIT</button>
            </div>
          )}
        </div>
      ))}
...
const Admin: React.FC = () => {
  ...
  const sbBtn: React.CSSProperties = {
    ...
    border: "none", background: "transparent", color: "var(--c-black)", cursor: "pointer",
      ...
      <div style={{
        ...
        {SIDEBAR_ITEMS.map(item => (
          <button key={item.id} onClick={() => setActiveTab(item.id)} style={{
            ...
            color: activeTab === item.id ? "var(--c-black)" : "var(--c-black)",
